...
$().ready( function() {
    ...
    $sort.click( function() {
        ...
        if ($orderProperty.val() == orderProperty) {
            if ($orderDirection.val() == "false") {
                $orderDirection.val("true")
            } else {
                $orderDirection.val("false");
            }
        } else {
    ...
    if ($orderProperty.val() != "") {
        ...
        console.log("#listTable a[name='" + $orderProperty.val() + "']")
        if ($orderDirection.val() == "true") {
            $sort.removeClass("dropup").append('<span class="caret"/>');
        } else {
            $sort.addClass("dropup").append('<span class="caret"/>');
        }
        console.log("end:"+$sort.html())
    }else{
        console.log("order info is empty");
    }
    ...
    $listForm.submit(function() {
        ...
        if ($searchValue.length > 0 && $searchValue.val() != "" && $searchProperty.val() == "") {
