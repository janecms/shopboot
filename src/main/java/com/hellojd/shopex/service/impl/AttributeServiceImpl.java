...
public class AttributeServiceImpl extends ShopBaseServiceImpl<AttributeRepository,Attribute> implements AttributeService{
    ...
    public void save(AttributeBean attribute) {
        ...
        if (CollectionUtils.isNotEmpty(options)) {
            for (String option : options) {
                this.attributeOptionRepository.insert(new AttributeOption(id,option));
            }
        } else {
    ...
    public void update(AttributeBean attribute) {
        ...
        BeanUtils.copyProperties(attribute, attributePO,new String[] { "propertyIndex", "productCategory" });
        this.baseMapper.updateById(attributePO);
        final List<String> requests = attribute.getOptions();
        List<String> options = attributePO.getOptions();
        this.updateRefResult(requests,options,new RefResultUpdater<String>(){
            @Override
            public void execute(RefResult<String> refResult) {
                refResult.deletingSet.forEach(option ->{
                    attributeOptionRepository.delete(new EntityWrapper<>(new AttributeOption(attribute.getId(),option)));
                });
                refResult.savingSet.forEach(option ->{
                    attributeOptionRepository.insert(new AttributeOption(attribute.getId(),option));
                });
            }
        });
    }
